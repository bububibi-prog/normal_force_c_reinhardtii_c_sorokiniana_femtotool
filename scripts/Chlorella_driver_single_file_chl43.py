...
analyzer.detect_contact_slope(start_noise = 15, r_noise = 0.2, min_noise = 70,threshold_sigma=5,plot_true = True) # Good
analyzer.subtract_baseline(plot_true = True) # Good

analyzer.check_normality_slopes_baseline() # Good, Normality confirmed
analyzer.cutoff2(min_fraction=0.1,drop_limit = 4,factor_stddev = 3, plot_true=True) # True peak start as soon as backwards from peak we get inside 3 times stddev of noise, this means from thereon we are within 99.7% of noise datapoints (normal distribution), conservative. Good
...
outpath = Path("saved_single_analyzers") / "Chl43_single_analyzer.pkl"
