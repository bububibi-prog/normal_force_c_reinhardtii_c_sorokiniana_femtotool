...
analyzer.smooth_force_time(med_kernel = 11, sg_window= 21, sg_order= 2,plot_true = True) # Good
...
analyzer.detect_contact_slope(start_noise = 30, r_noise = 0.6, min_noise = 70,threshold_sigma=5,plot_true = True) # Good
analyzer.subtract_baseline(plot_true = True) # Good

analyzer.check_normality_slopes_baseline() # Good, Normality confirmed
analyzer.cutoff2(min_fraction=0.2,drop_limit = 6,factor_stddev = 3, plot_true=True) # True peak start as soon as backwards from peak we get inside 3 times stddev of noise, this means from thereon we are within 99.7% of noise datapoints (normal distribution), conservative. Good
...
outpath = Path("saved_single_analyzers") / "Chl12_single_analyzer.pkl"
